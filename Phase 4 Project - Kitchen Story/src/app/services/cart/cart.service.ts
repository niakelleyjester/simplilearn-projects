...
export class CartService {
  ...
  //private cart:Cart = new Cart(); //Problem with this approach - anytime you refresh the page, the Cart will be removed
                                  //we want the Cart data to be persistent. we can use the local storage to accomplish this for now
  ...
  /* Will implement once we connect to the backend using HTTP
  //let's inject the dependency on HttpClient by adding it to the constructor of the Cart Service
  ...
  */
  ...
  removeFromCart(productId:number):void{
    this.cart.items =
    this.cart.items.filter(item => item.product.productId != productId); //removing the product with productId from the cartItems array
    this.setCartToLocalStorage(); //since we changed the Cart contents, we set cart to local storage for data persistence
   ...
   * Input Parameters: none
   * Return Type: Observable<Cart> - will return a Cart that is an observable object (real-time fetch of the data)
   * Purpose: create a method to fetch the Cart object from the API
   * ******************************************* */
  /* Will implement once we connect to the backend */
  /*getCartObservable():Observable<Cart>{
